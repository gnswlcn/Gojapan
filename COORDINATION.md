...
### [2026-04-13] 캐릭터 표정 연동 요청

**내용:**  
`episode_info.character` 필드 추가. 각 에피소드마다 전담 캐릭터와 4가지 표정 상태 정의.

| 에피소드 | 캐릭터 | idle | talking | correct | wrong |
|---|---|---|---|---|---|
| ep001 | 타로 (편의점 알바) | 🙂 | 😄 | 🤩 | 😤 |
| ep002 | 유이 (면세 카운터) | 🙂 | 😊 | 😌 | 🥶 |
| ep003 | 켄지 (식당 홀) | 😅 | 😃 | 😊 | 😑 |
...
**앱 수정 요청:**
- 점원 말풍선 옆(또는 위)에 캐릭터 표정 이모지 표시
- 상태 전환:
  - 턴 시작 → `idle`
  - TTS 재생 중 → `talking`
  - 정답 선택 → `correct` (짧은 애니메이션 후 idle 복귀)
  - 오답 선택 → `wrong` (셰이크 애니메이션과 동시 / 재시도 후 idle 복귀)
- `character.id` 를 키로 사용해 추후 실제 일러스트 에셋으로 교체 가능하게 설계 권장
